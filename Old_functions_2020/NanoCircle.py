...
def FULL(file_name,bamfile,bamname,savedir,mapQ):
    ...
    Complex_col_no = Complex_full_length(file_name, bamfile,bamname, mapQ)
    ...
    Simple_bed.saveas("{0}Simple_circles_bdg.bed".format(savedir))
    Simple_read_bed = bt.BedTool.from_dataframe(Simple_read)
    Simple_read_bed.saveas("{0}Simple_reads_bdg.bed".format(savedir))

    bedtest = bt.BedTool.from_dataframe(Complex_df)
    bedtest.saveas("{0}Complex_circles_bdg.bed".format(savedir))
    bedtest = bt.BedTool.from_dataframe(read_df_full)
    bedtest.saveas("{0}Complex_reads_bdg.bed".format(savedir))
...
def BED_Coverage(dir,bamfile,bamname,overlap,savedir,mapQ):
    ...
    for region in Merged:
        ...
        if circ_type == 1:
            ...
            df = Complex_circ_BED(circle_dict, circ_coord, circ_chr, 12, Complex_count, circ_type, bamname)
            Complex_df = Complex_df.append(df, sort=False).fillna("nan")
    ...
    Simple_bed = bt.BedTool.from_dataframe(Simple_circ)
    Simple_bed.saveas("{0}Simple_circles_cov.bed".format(str(savedir)))
    Simple_read_bed = bt.BedTool.from_dataframe(Simple_read)
    Simple_read_bed.saveas("{0}Simple_reads_cov.bed".format(str(savedir)))
    bedtest = bt.BedTool.from_dataframe(Complex_df)
    bedtest.saveas("{0}Complex_circles_cov.bed".format(str(savedir)))
    bedtest = bt.BedTool.from_dataframe(read_df_full)
    bedtest.saveas("{0}Complex_reads_cov.bed".format(str(savedir)))

ID = "BC12"
dir = "/isdata/common/wql443/Thesis_2019/Analysis/3.6samples/{0}/".format(ID)
dir2 = os.mkdir("{0}/".format(ID))
bamfile = ps.AlignmentFile(dir+"{0}.aln_hg19.bam".format(ID),"rb")
FULL("/isdata/common/wql443/Thesis_2019/Analysis/3.6samples/{0}/{0}.ge_mean5.bdg".format(ID),bamfile,"{0}.aln_hg19.bam".format(ID),"{0}/".format(ID),60)
BED_Coverage(dir,bamfile,"{0}.aln_hg19.bam".format(ID),1000,"{0}/".format(ID),60)