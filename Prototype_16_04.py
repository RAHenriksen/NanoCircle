...
def Complex(bamfile,mapQ,reg, start, end):
    ...
    for read in Read_list:
        ...
        for Tagelem in Tag:
            ...
            if chrom == reg and start - overlap <= pos_start <= end + overlap \
                    and start - overlap <= pos_end <= end + overlap:
                continue
            #else append the coordinates
            ...
            if Coord_list != []:
                ...
                #print("COoord",Coord_list)
                if len(Coord_list) == 2:
                    Complex_dict[read.query_name].extend(
                        [chroms[0], min(Coord_list), max(Coord_list)])
                else:
                    Grouped_list = Grouping(Coord_list, overlap)
                    #print(Grouped_list)
                    print("chroms",chroms)
                    for i in range(len(Grouped_list)):
                        # print("potential interval",min(Grouped_list[i]), max(Grouped_list[i]))
                        if len(chroms) < len(Grouped_list):
                            Complex_dict[read.query_name].extend(
                                [chroms[0], min(Grouped_list[i]), max(Grouped_list[i])])
                            #print("coord_compl",Complex_dict)
                        elif len(chroms) > len(Grouped_list):
                            #print("Chroms_compl", Complex_dict)
                            Complex_dict[read.query_name].extend(
                                [chroms[i * 2], min(Grouped_list[i]), max(Grouped_list[i])])
            elif Coord_list == []:
...
"""
def Complex3(bamfile,mapQ,reg, start, end):
    Sing_dict = Single_coord(bamfile, mapQ, reg, start, end,0)
    coord1 = list(Sing_dict.values())[0][0]
    coord2 = list(Sing_dict.values())[0][-1]
    overlap = coord2-coord1
    #print("overlap",overlap)
    #print(list(Sing_dict.values())[0])
